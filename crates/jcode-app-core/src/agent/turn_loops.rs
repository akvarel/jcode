...
impl Agent {
    ...
    pub(super) async fn run_turn(&mut self, print_output: bool) -> Result<String> {
        ...
        loop {
            ...
            // Personal memory remains non-blocking. GraphCompact is retrieved
            // synchronously so the compiled code graph affects this provider call.
            let memory_pending = self
                .build_memory_prompt_for_current_turn(std::sync::Arc::clone(&messages), None)
                .await;
